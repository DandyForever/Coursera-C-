...
// конструктор по умолчанию: список элементов пока пуст
    AirportCounter()
    {
        for (int i = 0; i < static_cast<size_t>(TAirport::Last_); i++) {
            data_[i] = {static_cast<TAirport>(i), 0};
        }
    }
...
// конструктор от диапазона элементов типа TAirport
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        for (auto item = begin; item != end; item++) {
            data_[static_cast<size_t>(*item)].second++;
//            data_[static_cast<size_t>(*item)].first = static_cast<TAirport>(*item);
        }
    }

// получить количество элементов, равных данному
    size_t Get(TAirport airport) const { return data_.at(static_cast<size_t>(airport)).second; }

// добавить данный элемент
    void Insert(TAirport airport) { data_[static_cast<size_t>(airport)].second++; }

// удалить одно вхождение данного элемента
    void EraseOne(TAirport airport) { data_[static_cast<size_t>(airport)].second--; }
...
// упорядоченных по аэропорту
    Items GetItems() const { return data_; }
