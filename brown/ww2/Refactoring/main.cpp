...
Json::Document XmlToJson(const Xml::Document& doc) {
    ...
    Json::Document docu(Json::Node(std::move(result)));
...
Xml::Document JsonToXml(const Json::Document& doc, std::string root_name) {
    ...
    for (const auto& n : doc.GetRoot().AsArray()) {
        int attr_amount = n.AsMap().at("amount").AsInt();
...
void TestJsonToXml() {
    ...
    const Json::Document json_doc{Node(std::vector<Json::Node>{
