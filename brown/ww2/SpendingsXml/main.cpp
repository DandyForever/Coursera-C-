...
vector<Spending> LoadFromXml(istream& input) {
    auto doc = Load(input);
    auto& records = doc.GetRoot().Children();
    auto result = vector<Spending>();
    result.reserve(records.size());
    for (auto& item : records) {
        auto category = item.AttributeValue<string>("category");
        auto amount = item.AttributeValue<int>("amount");
