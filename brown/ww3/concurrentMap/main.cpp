...
//  static_assert(is_integral_v<K>, "ConcurrentMap supports only integer keys");
    ...
    WriteAccess operator[](const K& key) {
        { lock_guard g(mutexes_[get_bucket_(key)]); }
        lock_guard g(ms_[get_bucket_(key)]);
        return {data_[get_bucket_(key)][key], lock_guard(mutexes_[get_bucket_(key)])};
    }
    ReadAccess At(const K& key) const {
        { lock_guard g(mutexes_[get_bucket_(key)]); }
        lock_guard g(ms_[get_bucket_(key)]);
        return {data_[get_bucket_(key)].at(key), lock_guard(mutexes_[get_bucket_(key)])};
    }

    bool Has(const K& key) const {
        return static_cast<bool>(data_[get_bucket_(key)].count(key));
    }
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (int i = 0; i < bucket_count_; i++) {
            lock_guard vg(ms_[i]);
            lock_guard vgg(mutexes_[i]);
            for (auto& jtem : data_[i]) {
                result[jtem.first] = jtem.second;
            }
        }
        return result;
    }
...
private:
    ...
    size_t get_bucket_(const K& key) const {
