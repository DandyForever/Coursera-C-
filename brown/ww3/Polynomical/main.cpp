...
private:
  ...
  void Shrink() {
    while (coeffs_.size() > 1 && std::abs(coeffs_.back()) <= static_cast<T>(1e-6)) {
      coeffs_.pop_back();
    }
  }
    ...
    public:
        ...
        IndexProxy& operator=(const T& value) {
            if (degree_ > poly_.coeffs_.size() && value != static_cast<T>(0)) {
                poly_.coeffs_.resize(degree_ + 1);
                poly_.coeffs_[degree_] = value;
            } else
                poly_.coeffs_[degree_] = value;
            if (std::abs(value) <= static_cast<T>(1e-6))
                poly_.Shrink();
            return *this;
        }
        friend ostream& operator<<(ostream& os, const IndexProxy& proxy) {
            os << proxy.value;
            return os;
        }
    private:
...
//  }
  ...
  const T& operator[](size_t degree) const {
      return degree < coeffs_.size() ? coeffs_[degree] : zero_val;
  }
