...
class DomainChecker {
public:
    explicit DomainChecker(istream& input = cin) {
        banned_domains = ReadDomains(input);
        domains_to_check = ReadDomains(input);
    }

    void print_check_result(ostream& output = cout) {
...
private:
    ...
    static vector<string> ReadDomains(istream& input = cin) {
        size_t count;
        input >> count >> std::ws;
...
int main() {
    DomainChecker checker(cin);
    checker.print_check_result();
    return 0;
}
