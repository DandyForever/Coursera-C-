...
struct BulkInfo {
    ...
    BulkInfo operator + (const BulkInfo& info) const {
        return { income + info.income, outcome + info.outcome };
    }
};
...
public:
    ...
    BulkLinearUpdater(const BulkMoneySpender& sub)
            : sub_(sub)
    {}
    ...
    BulkInfo Collapse(BulkInfo origin, IndexSegment segment) const {
        return { origin.income * tax_.factor + add_.delta * segment.length(),
                 origin.outcome + sub_.delta * segment.length() };
    }
