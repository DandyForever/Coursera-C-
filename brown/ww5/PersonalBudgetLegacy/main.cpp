...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, tree_sub_values, tree_sub;

void Init() {
    tree_values.fill(0);
    tree_add.fill(0);
    tree_sub_values.fill(0);
    tree_sub.fill(0);
    tree_factor.fill(1);
}
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            ...
            double percentage;
            cin >> percentage;
            Multiply(1, 0, DAY_COUNT_P2, idx_from, idx_to, 1 - percentage / 100);
        }
