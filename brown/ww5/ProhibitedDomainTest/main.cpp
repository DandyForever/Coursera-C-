...
void TestEmptyStringFirst() {
    stringstream input;
    input << "1\nru\n";
    const vector<Domain> test_domains = ReadDomains(input);
    ASSERT_EQUAL(test_domains[0], Domain("ru"));
}

void TestEmptySubdomainFirst() {
    stringstream input;
    input << "1\nru\n";
    const vector<Domain> test_domains = ReadDomains(input);
    ASSERT_EQUAL(test_domains[0].GetPartCount(), 1);
}

void TestReversedSubdomains() {
    Domain test_domain("ru.com");
    auto reversed_parts = test_domain.GetReversedParts();
    ASSERT_EQUAL(*reversed_parts.begin(), "com")
}

void TestDomainIsSelfSubdomain() {
    Domain test_domain("ru");
    ASSERT(IsSubdomain(test_domain, test_domain));
}

void TestIsSubdomainOrder() {
    Domain test_domain("ru"), test_subdomain("com.ru");
    ASSERT(IsSubdomain(test_subdomain, test_domain))
    ASSERT(!IsSubdomain(test_domain, test_subdomain))
}

void TestSubdomainAbsorb() {
    vector<Domain> banned_domains;
    banned_domains.emplace_back("ru");
    banned_domains.emplace_back("ra.com.ru");
    banned_domains.emplace_back("tan.com.ru");
    vector<Domain> domains_to_check;
    domains_to_check.emplace_back("tap.com.ru");
...
void TestGoodBad() {
    ...
    vector<Domain> domains_to_check;
    domains_to_check.emplace_back("tap.com.ru");
    stringstream out, answer;
    answer << "Bad\n";
    PrintCheckResults(CheckDomains(banned_domains, domains_to_check), out);
    ASSERT_EQUAL(out.str(), answer.str());
}

int main() {
    TestRunner tr;
    RUN_TEST(tr, TestEmptyStringFirst);
    RUN_TEST(tr, TestEmptySubdomainFirst);
    RUN_TEST(tr, TestReversedSubdomains);
    RUN_TEST(tr, TestDomainIsSelfSubdomain);
    RUN_TEST(tr, TestIsSubdomainOrder);
    RUN_TEST(tr, TestSubdomainAbsorb);
