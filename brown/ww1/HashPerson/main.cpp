...
struct AddressHasher {
    size_t operator() (const Address& address) const {
        size_t hc = shash(address.city);
        size_t hs = shash(address.street);
        size_t hb = ihash(address.building);
        size_t x = 31;
        return hc * x * x + hs * x + hb;
    }
...
struct PersonHasher {
    size_t operator() (const Person& person) const {
        size_t hn = shash(person.name);
        size_t hh = ihash(person.height);
        size_t wh = ihash(person.weight);
        size_t ah = ahash(person.address);
        size_t x = 31;
        return hn * x * x * x + hh * x * x + wh * x + ah;
    }
